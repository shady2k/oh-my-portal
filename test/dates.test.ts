...
describe('editorial dates', () => {
  it('uses an unpadded day and a Russian month, optionally with a year', () => {
    const date = new Date('2026-09-08T00:00:00Z');
    expect(displayDate(date, false)).toBe('8 сентября');
    expect(displayDate(date)).toBe('8 сентября 2026');
  });
